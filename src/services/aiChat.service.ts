...
export class AIChatService {
  ...
  async processMessage(
    ...
    const recentTurns = await AIOperationLog.getRecentConversationHistory(
      userId,
      domainId,
      AIChatService.CONVERSATION_MAX_MESSAGES
    );
    const zaiHistory: ZAIMessage[] = recentTurns.flatMap(turn => {
      const msgs: ZAIMessage[] = [{ role: 'user', content: turn.userMessage }];
      if (turn.aiResponse) {
