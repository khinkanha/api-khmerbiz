...
const BLOCK_THRESHOLD = config.aiGuardrails.injectionBlockThreshold;
const SANITIZE_THRESHOLD = config.aiGuardrails.injectionSanitizeThreshold;
 ...
 */
function stripZeroWidthChars(str: string): string {
  let result = '';
  for (const char of str) {
    const code = char.codePointAt(0)!;
    // Skip zero-width and invisible chars
    if (
      (code >= 0x200B && code <= 0x200F) || // zero-width space, non-joiner, etc.
 ...
 */
export function scoreInjectionRisk(message: string): { score: number; matches: string[] } {
 ...
 */
export function sanitizeAIInput(rawMessage: string): {
